...
def consider_target(attacker, target, aiSearchingTgt=False):
	if attacker.npc_flags_get() & ONF_NO_ATTACK:
		return 0
	...
	if not target.is_critter():
		...
		dist = target.distance_to(attacker)
		if dist > 125.0: 
			if do_print: debug_print("consider_target:too far {} > 125.0".format(dist))
			debug.breakp("too far 125")
			return 0

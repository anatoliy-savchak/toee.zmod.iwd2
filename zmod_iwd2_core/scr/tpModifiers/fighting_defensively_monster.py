...
def Fighting_Defensively_Monster_OnBeginRound(attachee, args, evt_obj):
	assert isinstance(attachee, toee.PyObjHandle)
	assert isinstance(args, tpdp.EventArgs)
	assert isinstance(evt_obj, tpdp.EventObjD20Signal)
	print("Fighting_Defensively_Monster_OnBeginRound for {}".format(attachee))
	args.condition_remove()
	return 0
...
modObj.AddHook(toee.ET_OnBeginRound, toee.EK_NONE, Fighting_Defensively_Monster_OnBeginRound, ())
